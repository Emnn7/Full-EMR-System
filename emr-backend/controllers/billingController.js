...
exports.getAllBillings = catchAsync(async (req, res, next) => {
  ...
  if (req.user.role === ROLES.RECEPTIONIST) {
    ...
    query = Billing.find({ patient: req.user._id });
  } else {
    query = Billing.find();
  }
...
exports.getBilling = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    billing.patient.toString() !== req.user._id.toString()
  ) {
...
exports.generateInvoice = catchAsync(async (req, res, next) => {
  ...
  if (
    req.user.role !== ROLES.ADMIN &&
    req.user.role !== ROLES.RECEPTIONIST &&
    billing.patient._id.toString() !== req.user._id.toString()
  ) {
    return next(
...
exports.createRegistrationBilling = catchAsync(async (req, res, next) => {
  ...
  const billingData = {
    ...
    total: 100,
    status: 'pending',
    paymentType: 'registration',  // Ensure this is set
    createdBy: req.user._id,
    createdByModel: capitalizeFirstLetter(req.user.role)
  };
...
// Update getUnpaidBills to include all types
